...
bool SimpleRaycaster::init( int width, int height )
{
	...
	m_opacity_tex    .Image( 0, GL_ALPHA8, 4, 0, GL_ALPHA, GL_FLOAT, NULL );
	m_lut_tex.Image( 0, GL_RGBA32F, 94,150,150, 0, 
...
void SimpleRaycaster::setLookupTable()
{
	...
	std::string sen1;
	int count =0;
  ifstream myfile ("E:/SemesterIII/Thesis/VolVis_build/Results/SVMResultsVector-BFremoved.dat");
  if (myfile.is_open())
  {

    while ( getline (myfile,sen1) )
    {
		count++;
		//std::size_t found = sen1.find('e');
		double f;
	...
	buffer.push_back(f);
		...
		{
			buffer.push_back(0.08);
			buffer.push_back(0.91);
			buffer.push_back(0.05);
			buffer.push_back(0.05);
		}
		else if(f==3)
		{
  ...
  cout<<"TOTAOL " <<count<<endl;
  opacity.push_back(0);
  opacity.push_back(0.1);
  opacity.push_back(0.5);
  opacity.push_back(1);

  
	m_lut_tex.SetWrapMode( GL_CLAMP_TO_EDGE );  
	m_lut_tex.SetFilterMode( GL_LINEAR );
	m_lut_tex.SubImage( 0, 0,0,0, 94,150,150,
	                            GL_RGBA, GL_FLOAT, (void*)(&buffer[0]) );
	m_opacity_tex.Image( 0, GL_ALPHA8,4, 0, GL_ALPHA, GL_FLOAT, 
		             (void*)(&opacity[0]) );
