...
void main(void)
{
	...
	for( int i=0; i < numSteps; ++i )
	{
		...
		vec3 p_color_red = color_red * src.r * opacity_red;
		vec3 p_color_green = color_green *src.g * opacity_green;
		vec3 p_color_blue = color_blue*src.b * opacity_blue;
		vec3 p_color_yellow = color_yellow*src.a * opacity_yellow;
		...
		vec3 lightDirection = normalize(uLightPosition - vec3(rayPosition));
		//dst = dst + (1.0 - dst.a)*final_color;
		 //dst = dst + (1.0 - dst.a) * (shading_diffuse( normalize(normal_vector), lightDirection  ) * weighted_Opacity);
